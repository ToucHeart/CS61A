...
def swine_swap(player_score, opponent_score):
    ...
    """
    a = str(pow(3, player_score + opponent_score))
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    >>> f3 = f2(20, 10)
    >>> f4 = f3(20, 20)
    >>> f5 = f4(20, 35) # Player 1 gets 17 points; not enough for a new high
    ...
    def commentary(newscore0, newscore1):
        if who == 0:
            newscore = newscore0
        else:
            newscore = newscore1
        new_high = newscore - last_score
        if new_high > running_high:
            print(f"{new_high} point(s)! That's a record gain for Player {who}!")
        return announce_highest(who, newscore, max(running_high, new_high))
...
def max_scoring_num_rolls(current_score=0, dice=six_sided, trials_count=1000):
    ...
    max_num = -1
    for i in range(1, 11):
        tmp = make_averaged(roll_dice, trials_count)(current_score, i, dice)
        if tmp > max_num:
            max_num = tmp
            ans = i
    return ans
