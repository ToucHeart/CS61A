...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while p and not p.is_hive:
            if len(p.bees) and self.min_range <= distance <= self.max_range:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        while p:
            if p.ant:
                if p.ant not in self.buffer:
                    p.ant.buff()
                    self.buffer.append(p.ant)
                if p.ant.is_container and p.ant.ant_contained:
                    if p.ant.ant_contained not in self.buffer:
                        p.ant.ant_contained.buff()
                        self.buffer.append(p.ant.ant_contained)
            p = p.exit
    ...
    def remove_from(self, place):  # can't be removed
        pass
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.slowturn == 0 or (self.slowturn > 0 and gamestate.time % 2 == 0):
            if self.blocked():
                self.sting(self.place.ant)
            elif self.is_scared:
                if destination and not destination.is_hive:
                    self.move_to(destination)
                self.scare_step += 1
                if self.scare_step == 2:
                    self.is_scared = False
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        return self.damage - distance * 0.25 - 0.0625 * self.insects_shot
