...
class Polynomial:
    ...
    def __repr__(self):
        if self.l == [0]:
            ...
            tmp = self.l
            while tmp[len(tmp) - 1] == 0:
                tmp.pop(len(tmp) - 1)
            return f"Polynomial({tmp})"
    ...
    def __str__(self):
        ...
        for x in range(1, len(self.l)):
            string = string + " + " + str(self.l[x]) + '*x^' + str(x)
        return string

    def __mul__(self, other):
        newlen = len(self.l) + len(other.l)
        ans = [0] * newlen
        for i in range(len(self.l)):
            for j in range(len(other.l)):
                ans[i + j] += self.l[i] * other.l[j]
        return Polynomial(ans)

    def __add__(self, other):
        x = min(len(self.l), len(other.l))
        ans = []
        for i in range(x):
...
def reverse(lnk):
    ...
    if lnk.rest == Link.empty:
        return lnk
    elif lnk.rest.rest == Link.empty:
        tmp = lnk.rest
        lnk.rest = Link.empty
        tmp.rest = lnk
        return tmp
    else:
        left, mid, last = lnk, lnk.rest, lnk.rest.rest
        left.rest = Link.empty
...
class Tree:
    ...
    def __eq__(self, other):  # Does this line need to be changed?
        ...
        if not self.is_leaf() and other.is_leaf():
            return False
        if self.is_leaf() and other.is_leaf() and self.label != other.label:
            return False
        if len(self.branches) != len(other.branches):
            return False
        ans = (self.label == other.label)
        for i in range(len(self.branches)):
            ans = ans and Tree.__eq__(self.branches[i], other.branches[i])
        return ans
...
def generate_paths(t, value):
    ...
    if t.label == value:
        yield [value]
    for x in t.branches:
        for y in generate_paths(x, value):
            yield [t.label] + y
...
def count_coins_tree(change, denominations):
    ...
    if branches:
        ans = Tree(str(change) + ', ' + str(denominations))
...
def balance_tree(t):
    ...
    if t.is_leaf():
        ...
        for x in t.branches:
            balance_tree(x)
        maxlabel = max([x.sumoftree for x in t.branches])
        for x in t.branches:
            if x.sumoftree < maxlabel:
                x.label += (maxlabel - x.sumoftree)
                x.sumoftree += (maxlabel - x.sumoftree)
        t.sumoftree += (t.label + len(t.branches) * t.branches[0].sumoftree)
        return
